...
before assembling.  Binning methods began with many k-mer techniques [@tags:McHardy @tags:nbc @tags:Phymm] 
...
naturally using BLAST [@tags:blast], followed by other machine learning techniques 
such as SVMs [@tags:McHardy @tags:McHardy2], naive Bayesian classifiers [@tags:nbc], etc. to classify 
...
estimators (a.k.a diversity profilers) are MetaPhlan[@tags:Metaphlan], (WGS)Quikr[@tags:wgsquikr],
and some configurations of tools like OneCodex[@tags:onecodex] and LMAT[@tags:lmat].  While one
...
relative abundances at faster rates [@tags:Vervier] and as of this writing, there
...
annotation of sequence reads [@tags:yok @tags:Soueidan]. However, the focus on
...
between these organisms and host/environment phenotypes [@tags:Guetterman].  One of
the first attempts was a survey of supervised classification methods for
microbes->phenotype classification[@tags:Knights], followed by similar studies
that are more massive in scale [@tags:Stratnikov, @tags:Segata].  There have been
techniques that bypass the taxonomic classification step altogether [@tags:Ding]
, (sequence composition to phenotype classification).  Also, researchers have
looked into how feature selection can improve classification [@tags:Liu @tags:Segata],
and techniques have been proposed that are classifier-independent
[@tags:Ditzler,@tags:Ditzler2].
...
successfully to gene annotation (e.g. Orphelia [@tags:Hoff] and FragGeneScan [@doi:10.1093/nar/gkq747]), 
which usually has plenty of training examples.  Representations (similar to Word2Vec [@tags:Word2Vec] in 
natural language processing) for protein family classification has been introduced and classified 
with a skip-gram  neural network [@tags:Asgari].  Recurrent neural networks show good performance for 
homology and protein family identification [@tags:Hochreiter @tags:Sonderby].  Interestingly, 
...
type of NN [@tags:Abe].  Essinger et al. use ART, a neural network algorithm called Adaptive 
...
[@tags:Salzberg] have performed better than these early genome binners.  Also, neural networks 
can be slow, and therefore, have had limited use for reference-based taxonomic 
classification, with TAC-ELM [@tags:TAC-ELM] being the only NN-based algorithm to taxonomically 
...
forests [@tags:Mrzelj].  However, this study performed 10-fold cross-validation on 3000 sequences in total. 
...
was able to classify wound severity from microbial species present in the wound [@tags:Kizek].
...
[@tags:Ditzler3] .  Besides classifying the samples appropriately, Ditzler shows
